  ...
  <div>
    <div class="page_content_wrap">
      <el-form ref="form" :model="form" :form="form" :rules="rules" label-width="80px" label-position="right"
        ...
        <div class="search-group">
          <div class="search-content">
            ...
            <el-form-item label="选择时间" prop="deleteEndTime">
              <div style="min-width: 286px;">
                <el-date-picker v-model="form.deleteStartTime" :picker-options="disableStartDate" type="datetime"
                  value-format="yyyy-MM-dd HH:mm:ss" format="yyyy-MM-dd HH:mm" placeholder="开始时间" />
                <span> ~ </span>
                <el-date-picker v-model="form.deleteEndTime" type="datetime" :picker-options="disableEndDate"
                  value-format="yyyy-MM-dd HH:mm:ss" format="yyyy-MM-dd HH:mm" placeholder="结束时间" />
              </div>

            </el-form-item>
            <!-- 优化代码 对俩个时间分别校验 -->
            <el-form-item label="留痕状态" prop="recordType">
              <div class="recordTypeStyle">
                <el-select v-model="form.recordType" filterable clearable placeholder="请选择" @change="changeRecordType">
                  <el-option v-for="item in recordList" :key="item.value" :label="item.label" :value="item.value" />
                </el-select>
              </div>
            </el-form-item>
          ...
          <div class="search-action">
            ...
            <el-button class="right golden" type="primary" @click="listDeletedObjects(1)">查询</el-button>
          </div>
        </div>
      </el-form>
      <TableData :columns="columns" :loading="loading" :table-data="tableData" :page-obj="page" @go-page="onGoPage">
        ...
        <el-table-column slot="deleteType" label="删除方式" width="120px">
          <template slot-scope="scope">
            <span>{{ deleteTypes[scope.row.deleteType] || '未知' }}</span>
          </template>
        </el-table-column>
        <el-table-column slot="recordType" label="删除状态" width="100px">
          <template slot-scope="scope">
        ...
        .then(() => {
          ...
          this.$store.state.user._gatewayS3.call({ path: '/', param: { restoreDeleted: '' }, data, method: 'POST', ignoreNotice: true }).then(res => {
            ...
            if (err.code == 'RestoreConflict') {
              ...
              this.$msg({ type: 'error', text: err.message })
            }
          })
        })
    }
