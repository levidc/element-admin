    ...
    <div class="page_content_wrap">
      <div class="mb_15 clearfix">
        ...
        <el-input v-model="searchVal" class="search_style search_btn right with_search mr_10" placeholder="存储桶名过滤"
          clearable @input="filterSearch" />
      </div>
      <DataTable :columns="columns" :table-data="tableData" :loading="loading" pagination
        :page-obj="{ pageSize: 10, currentPage: 1 }" :total="total" @renderPagination="getPageSearch">
        <template slot="action">
          <el-table-column min-width="100px" :label="$ts('page.action')">
            <template slot-scope="scope">
              <el-button class="blue" type="text" @click="viewLifeCycleRule(scope.row)">查看</el-button>
            </template>
          </el-table-column>
        </template>
      </DataTable>
