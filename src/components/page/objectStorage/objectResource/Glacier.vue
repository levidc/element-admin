  ...
  <div>
    <div class="page_content_wrap">
      ...
      <TableData ref="tab" :loading="loading" :row-key="rendeKey" :table-data="tableData" :columns="tableColumns"
        :page-obj="{ ...pageObj }" :sort-function="sortFunction" :expand-change="expandChange">
        ...
        <el-table-column slot="storageStatus" label="存储状态" prop="storageStatus" type="expand" width="300">
          <template slot-scope="scope">
            <el-table v-loading="scope.row.loading" :data="scope.row.statusList"
              ...
              <el-table-column prop="gatewayStatus" label="服务状态" align="center">
                <template slot-scope="row">
                  <span v-if="row.row.gatewayStatus" class="green">正常</span>
                  <span v-else class="red">异常</span>
                </template>
              </el-table-column>
              <el-table-column prop="resourceStatus" label="资源连接状态" align="center">
                <template slot-scope="row">
                  <!-- {{ row.row.resourceStatus}} -->
                  <span v-if="row.row.resourceStatus === 'OK'" class="green">
                    {{ storageTypeEnu(row.row.resourceStatus) }}
                  </span>
                  <span v-else-if="storageTypeEnu[row.row.resourceStatus]" class="red">
                    {{ storageTypeEnu(row.row.resourceStatus) }}
                  </span>
    ...
    <el-dialog :visible.sync="modelFormFlag" width="40%" :title="isAdd ? '创建资源' : '修改资源'"
      ...
      <el-form ref="form" :model="form" :rules="rules" label-width="160px">
        ...
        <el-row>
          <el-col :span="24">
            <el-form-item prop="userName" label="用户名">
              <el-input v-model.trim="form.userName" placeholder="请输入2-64位字符" clearable />
            </el-form-item>
          </el-col>
          <el-col :span="24">
            <el-form-item prop="password" label="密码">
              <el-input v-model.trim="form.password" type="password" placeholder="请输入2-64位字符" clearable />
            </el-form-item>
          </el-col>
        </el-row>
        <el-row>
          <el-col :span="24">
            <el-form-item prop="bucketName" label="存储桶名">
              <el-popover placement="top" width="350" trigger="hover" style="position:absolute;">
                ...
                <svg slot="reference" class="icon icon-question" aria-hidden="true" style="margin-left: -20px !important;">
                  <use xlink:href="#icon-question" />
                </svg>
              </el-popover>
        ...
        <el-row v-if="enableAdvancedConfig">
          <el-col :span="12">
            <el-form-item prop="s3SdkVersion" label="s3 SDK版本">
              <el-select v-model="form.extendInfo.s3SdkVersion" :disabled="!isAdd">
                <el-option v-for="item in s3SdkVersion" :key="item" :value="item" :label="item" />
              </el-select>
            </el-form-item>
          </el-col>
          <el-col :span="12">
            <el-form-item props="s3SignatureVersion" label="s3 签名版本">
              <el-select v-model="form.extendInfo.s3SignatureVersion" :disabled="!isAdd">
                <el-option v-for="item in s3SignatureVersion" :key="item" :value="item" :label="item" />
              </el-select>
            </el-form-item>
          </el-col>
        </el-row>
...
export default {
  data () {
    return {
      ...
      tableColumns: [
        ...
        { slot: 'url', show: true },
        {
          prop: 'bucketName', title: '存储桶名', show: true, sortable: 'custom', minWidth: '200px'
        },
        {
          slot: 'storageStatus', title: '存储状态', show: true, formatter: (row, _, val) => {
  ...
  methods: {
    ...
    expandChange (val, param) {
      ...
      if (param.length) {
        clearTimeout(this.timer)
        if (!val.statusList.length) {
          val.loading = true
          const {
            resourceId
          } = val
          getResourceStatus({ resourceId }).then(res => {
            val.statusList = res.data || []
          }).finally(() => {
            val.loading = false
          })
        }
