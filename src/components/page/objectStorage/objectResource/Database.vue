  ...
  <div>
    ...
    <el-dialog :visible.sync="modelFormFlag" width="40%" :title="isAdd ? '创建资源' : '修改资源'"
      ...
      <el-form ref="form" :model="form" :rules="rules" label-width="160px">
        ...
        <el-row>
          <el-col :span="24">
            <el-form-item prop="bucketName" label="库结构名称">
              <!-- <el-popover
                placement="top"
                width="350"
                trigger="hover"
              ...
              <el-input v-model.trim="form.bucketName" placeholder="请输入库结构名称" :disabled="!isAdd" clearable />
            </el-form-item>
          </el-col>
          <el-col :span="24">
            <el-form-item prop="sign" label="数据库类型">
              <el-select v-model="form.sign" :disabled="!isAdd">
                <el-option v-for="item in DatabaseType" :key="item" :label="item" :value="item" />
              </el-select>
            </el-form-item>
  ...
  data () {
    return {
      ...
      rules: {
        ...
        url: [
          {
            ...
            validator: (rule, val, cb) => {
              const reg = /^((?:(?:25[0-5]|2[0-4]\d|((1\d{2})|([1-9]?\d)))\.){3}(?:25[0-5]|2[0-4]\d|((1\d{2})|([1-9]?\d))))(:([0-9]|[1-9]\d|[1-9]\d{2}|[1-9]\d{3}|[1-5]\d{4}|6[0-4]\d{3}|65[0-4]\d{2}|655[0-2]\d|6553[0-5]))?$/
              if (!val) {
                return cb('资源地址必填')
        ...
        bucketName: [
          {
            required: true, message: '库结构名称必填', trigger: ['blur', 'change']
          },
          {
            min: 2, max: 64, message: '请输入2-64位字符', trigger: ['blur', 'change']
          }
        ],
        userName: [
          {
      ...
      tableColumns: [
        ...
        { slot: 'url', show: true },
        {
          prop: 'bucketName', title: '库结构名称', show: true, sortable: 'custom', minWidth: '200px'
        },
        {
    ...
    init (flag = false) {
      ...
      listStorageResource({
        ...
        this.tableData = (res.data || []).filter(item => {
          return item.storageType === 'DATABASE'
        })
