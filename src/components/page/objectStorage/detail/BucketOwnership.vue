      ...
      <div class="bucket-detail-inner">
        <div class="bucket-panel">
          <div id="basic-info-field" class="param-box">
            <div class="param-hd">
              <h3>对象所有权 </h3>
              <a v-show="!editConfig && !loading" href="javascript:void(0);" class="link-edit"
                @click="editConfig = !editConfig">编辑</a>
              <p v-show="!editConfig">{{ valueMap[config] }}</p>

              <p v-if="config === 'BucketOwnerEnforced'" class="tip">
                ACL 已禁用。此存储桶中的所有对象均归此账户所有。仅可使用策略指定对此存储桶及其对象的访问权限。
              </p>
              <p v-if="config === 'BucketOwnerPreferred'" class="tip">
                ACL 已启用，并且可用于授予对此存储桶及其对象的访问权限。如果写入此存储桶的新对象指定了存储桶拥有者完全控制标准 ACL，则它们归存储桶拥有者所有。否则，它们归对象写入者所有。
              </p>
              <p v-if="config === 'ObjectWriter'" class="tip">
                ACL 已启用，并且可用于授予对此存储桶及其对象的访问权限。对象编写者依然为对象拥有者。
              </p>
              <div v-show="editConfig">
                <el-radio-group v-model="aclDisable" class="aclDisable">
                ...
                <el-radio-group v-show="aclDisable === 'on'" v-model="objectOwnership" class="enableACL">
                ...
                <div>
                  <el-button @click="editConfig = false">{{ $ts('page.cancel') }}</el-button>
